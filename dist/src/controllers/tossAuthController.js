...
exports.unlinkToss = exports.handleTossCallback = exports.completeTossRegistration = exports.loginWithToss = void 0;
...
const tossDecrypt_1 = require("../utils/tossDecrypt");
// Initialize mTLS HTTPS Agent
let httpsAgent = null;
try {
    const certPath = path_1.default.resolve(process.cwd(), 'src/config/certs/03.v_public.crt');
    const keyPath = path_1.default.resolve(process.cwd(), 'src/config/certs/03.v_private.key');
...
const loginWithToss = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // 1. 인가 코드로 엑세스 토큰 발급
        console.log('Toss Token Request - sending to generate-token with code length:', authorizationCode === null || authorizationCode === void 0 ? void 0 : authorizationCode.length);
        ...
        if (!user || needsRoleAddition) {
            ...
            if (selectedRole === 'TENANT' && !inviteCode) {
                ...
                const tempToken = jsonwebtoken_1.default.sign({ userKey: userKeyStr, name: tossUser.name, selectedRole }, env_1.JWT_SECRET, { expiresIn: '10m' });
                return res.json({
                    success: false,
            ...
            if (selectedRole === 'TENANT') {
                ...
                if (invitation.tenantId) {
                    ...
                    if (physicalTenant === null || physicalTenant === void 0 ? void 0 : physicalTenant.roomId)
                        resolvedRoomId = physicalTenant.roomId;
                }
            }
            ...
            const realName = (0, tossDecrypt_1.decryptTossData)(tossUser.name) || '토스유저';
            if (!user) {
                // 완전 신규 유저 생성
        ...
        const clientId = process.env.TOSS_CLIENT_ID;
        console.error('Toss Login Error (Full):', JSON.stringify((_e = error.response) === null || _e === void 0 ? void 0 : _e.data, null, 2) || error.message);
        console.error('Toss Login - Request URL:', (_f = error.config) === null || _f === void 0 ? void 0 : _f.url);
        console.error('Toss Login - TOSS_CLIENT_ID loaded:', clientId ? `${clientId.substring(0, 8)}...` : 'NOT SET');
        res.status(500).json({ error: '토스 로그인 처리 중 서버 오류가 발생했습니다.', details: (_g = error.response) === null || _g === void 0 ? void 0 : _g.data });
    }
});
exports.loginWithToss = loginWithToss;
/**
 * 토스 최초 로그인 시 추가 정보 입력 후 가입 완료
 * POST /api/auth/toss/complete-registration
 * body: { tempToken, inviteCode?, phone?, selectedRole }
 */
const completeTossRegistration = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(tempToken, env_1.JWT_SECRET);
        }
        catch (e) {
        ...
        const targetUsername = 'toss_' + userKeyStr;
        const existingUser = yield db_1.prisma.user.findFirst({ where: { username: targetUsername } });
        // 필수 필드 검증
        let resolvedRoomId = null;
        let resolvedLandlordId = null;
        let resolvedTenantId = null;
        if (role === 'TENANT') {
            if (!inviteCode)
                return res.status(400).json({ error: '초대코드를 입력해 주세요.' });
            const invitation = yield db_1.prisma.invitation.findUnique({ where: { code: inviteCode } });
            if (!invitation || invitation.isUsed || invitation.expiresAt < new Date()) {
                return res.status(400).json({ error: '유효하지 않거나 이미 사용된 초대코드입니다.' });
            }
        ...
        // JWT 발급
        const token = jsonwebtoken_1.default.sign({ id: user.id, username: user.username, roles: [role], currentRole: role, status: 'ACTIVE' }, env_1.JWT_SECRET, { expiresIn: '7d' });
        res.cookie('token', token, {
            httpOnly: true,
            secure: env_1.NODE_ENV === 'production',
            sameSite: 'strict',
    ...
    try {
        ...
        if (!user) {
            ...
            if (!currentRoles.includes(selectedRole)) {
                ...
                user.roles = [...currentRoles, selectedRole];
            }
            else {
