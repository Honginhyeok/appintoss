...
exports.PaymentBottomSheet = PaymentBottomSheet;
const react_1 = require("react");
function PaymentBottomSheet({ isOpen, onClose, onSelectTossTransfer, onSelectTossPay, onSelectManualDeposit }) {
    ...
    if (!isOpen && !show)
        return null;
    return (<div className={`fixed inset-0 z-[9999] flex flex-col justify-end transition-colors duration-300 ${show ? 'bg-black/40' : 'bg-transparent'}`} onClick={onClose}>
      <div className={`bg-white rounded-t-3xl w-full max-w-[480px] mx-auto overflow-hidden transition-transform duration-300 ease-out transform ${show ? 'translate-y-0' : 'translate-y-full'}`} onClick={(e) => e.stopPropagation()}>
        {/* 드래그 핸들 */}
        ...
        <div className="px-4 pb-8 flex flex-col gap-1">
          ...
          {/* 옵션 1: 토스 간편 송금 */}
          <div onClick={onSelectTossTransfer} className="flex items-center justify-between p-4 rounded-2xl active:bg-blue-50 transition-colors cursor-pointer group">
          ...
          {/* 옵션 2: 직접 입금 */}
          <div onClick={onSelectManualDeposit} className="flex items-center justify-between p-4 rounded-2xl active:bg-gray-100 transition-colors cursor-pointer group">
            <div className="flex items-center gap-4">
          ...
          <div onClick={onSelectTossPay} className="flex items-center justify-between p-4 rounded-2xl opacity-50 cursor-not-allowed pointer-events-none">
            <div className="flex items-center gap-4">
