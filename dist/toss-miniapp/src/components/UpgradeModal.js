...
const payment_widget_sdk_1 = require("@tosspayments/payment-widget-sdk");
function UpgradeModal({ isOpen, onClose, onSuccess }) {
    const paymentWidgetRef = (0, react_1.useRef)(null);
    const [isWidgetLoaded, setIsWidgetLoaded] = (0, react_1.useState)(false);
    const [isProcessing, setIsProcessing] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
        if (!isOpen)
            return;
        const initializeWidget = () => __awaiter(this, void 0, void 0, function* () {
            try {
                const widget = yield (0, payment_widget_sdk_1.loadPaymentWidget)("test_ck_D5GePWvyJnrK0W0k6q8gLzN97Eoq", "ANONYMOUS");
    ...
    const handlePayment = () => __awaiter(this, void 0, void 0, function* () {
        if (!paymentWidgetRef.current || isProcessing)
            return;
        setIsProcessing(true);
        try {
            // 1. 토스페이먼츠 위젯 가상 결제 호출 (PG)
            yield paymentWidgetRef.current.requestPayment({
            ...
            const res = yield fetch('/api/users/upgrade', {
                method: 'POST',
                headers: {
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-[10000] flex flex-col justify-end bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-t-3xl w-full max-w-md mx-auto overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
        {/* 모달 헤더 플래그 */}
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-3 mt-4">
            <button onClick={handlePayment} disabled={!isWidgetLoaded || isProcessing} className="w-full bg-blue-600 text-white font-bold text-[17px] py-4 rounded-xl disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-blue-700 transition-colors">
              {isProcessing ? '요청 중...' : '4,900원 정기결제 시작하기'}
            </button>
            <button onClick={onClose} className="w-full text-gray-500 font-medium text-[15px] py-3 hover:text-gray-700">
              다음에 할게요
            </button>
          </div>
        </div>
