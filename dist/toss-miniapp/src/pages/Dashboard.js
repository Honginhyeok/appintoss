...
function Dashboard() {
    ...
    const load = (0, react_1.useCallback)(() => __awaiter(this, void 0, void 0, function* () {
        setLoading(true);
        try {
            const [rRes, tRes, txRes] = yield Promise.all([
    ...
    const expectedIncome = tenants.reduce((s, t) => {
        if (!t.rentAmount)
            return s;
        if (t.rentType === 'YEARLY')
            return s; // 년세는 월별 예상에 미포함
    ...
    const getStatus = (t) => {
        if (!t.rentAmount)
            return 'PAID';
        const rentTxs = txs.filter(tx => { var _a; return tx.type === 'INCOME' && tx.category === 'RENT' && ((_a = tx.description) === null || _a === void 0 ? void 0 : _a.includes(t.name)) && tx.date.startsWith(thisMonth); });
        if (rentTxs.length > 0)
            return 'PAID';
    ...
    return (<div style={{ padding: '0 0 100px', minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{
        ...
        {loading ? (<Skeleton w="180px" h="36px" r="8px"/>) : (<div style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.5px' }}>
            {fmt(actualIncome)}
          </div>)}
        ...
        {/* 수입 달성률 바 */}
        {!loading && expectedIncome > 0 && (<div style={{ marginTop: '16px' }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        ].map(a => (<a key={a.href} href={a.href} style={{
      ...
      <div style={{ padding: '24px 20px 0' }}>
        ...
        {loading ? (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {[1, 2, 3].map(i => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#fff', borderRadius: '14px', padding: '14px' }}>
          ...
          </div>) : recentTxs.length === 0 ? (<div style={{ textAlign: 'center', padding: '32px', color: '#8b95a1', background: '#fff', borderRadius: '14px' }}>
            아직 거래 내역이 없습니다
          </div>) : (<div style={{ background: '#fff', borderRadius: '14px', overflow: 'hidden' }}>
            {recentTxs.map((t, i) => {
                var _a;
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '11px', color: '#8b95a1', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {((_a = t.room) === null || _a === void 0 ? void 0 : _a.name) || '공통'} · {new Date(t.date).toLocaleDateString('ko-KR')}
                  </div>
                </div>
      ...
      <div style={{ padding: '24px 20px 0', textAlign: 'center' }}>
        <div style={{ fontSize: '13px', color: '#b0b8c1' }}>
          {(user === null || user === void 0 ? void 0 : user.name) || (user === null || user === void 0 ? void 0 : user.username)}님, 오늘도 수고하셨습니다 ☕
        </div>
      </div>
    </div>);
}
