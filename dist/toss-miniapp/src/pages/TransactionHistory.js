...
function TransactionHistory() {
    const [txs, setTxs] = (0, react_1.useState)([]);
    ...
    const [category, setCategory] = (0, react_1.useState)('RENT');
    ...
    const load = (0, react_1.useCallback)(() => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const [tRes, rRes] = yield Promise.all([
                (0, env_1.apiFetch)('/api/transactions'),
                (0, env_1.apiFetch)('/api/rooms'),
            ]);
            if (tRes.ok)
                setTxs(yield tRes.json());
            if (rRes.ok)
    ...
    const handleDelete = (id) => __awaiter(this, void 0, void 0, function* () {
        if (!confirm('이 거래 내역을 삭제하시겠습니까?'))
            return;
        yield (0, env_1.apiFetch)(`/api/transactions/${id}`, { method: 'DELETE' });
        yield load();
    });
    ...
    return (<div style={{ padding: '24px', minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <tds_1.Button variant="primary" style={{ padding: '8px 14px', fontSize: '13px', width: 'auto', borderRadius: '10px' }} onClick={() => { resetForm(); setShowForm(true); }}>
          + 기록
        </tds_1.Button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {['ALL', 'INCOME', 'EXPENSE'].map(f => (<button key={f} onClick={() => setFilter(f)} style={{
      ...
      {/* 리스트 */}
      {loading ? (<div style={{ textAlign: 'center', padding: '40px', color: '#8b95a1' }}>불러오는 중...</div>) : filtered.length === 0 ? (<div style={{ textAlign: 'center', padding: '60px 20px', color: '#8b95a1' }}>
          <div style={{ fontSize: '40px', marginBottom: '12px' }}>📊</div>
          <div style={{ fontSize: '14px' }}>거래 내역이 없습니다</div>
        </div>) : (<div>
          {filtered.map(t => {
                var _a;
                return (<div key={t.id} style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid #f2f4f6' }}>
              <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: '12px', color: '#8b95a1', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {((_a = t.room) === null || _a === void 0 ? void 0 : _a.name) || '공통'} · {t.description || '-'} · {new Date(t.date).toLocaleDateString('ko-KR')}
                </div>
              </div>
      ...
      {/* 등록/수정 BottomSheet */}
      <tds_1.BottomSheet isOpen={showForm} onClose={() => { setShowForm(false); resetForm(); }} title={editId ? '거래 내역 수정' : '거래 기록 추가'}>
        {/* 수입/지출 토글 */}
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
          {['INCOME', 'EXPENSE'].map(v => (<button key={v} onClick={() => setType(v)} style={{
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <select value={category} onChange={e => setCategory(e.target.value)} style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '1px solid transparent', backgroundColor: '#f2f4f6', fontSize: '16px', outline: 'none', boxSizing: 'border-box' }}>
            {Object.entries(CATEGORIES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
        </div>

        <tds_1.TextField label="금액 (원) *" type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="500000"/>
        <tds_1.TextField label="날짜" type="date" value={date} onChange={e => setDate(e.target.value)}/>
        <tds_1.TextField label="설명 (메모)" value={description} onChange={e => setDescription(e.target.value)} placeholder="예: 홍길동 월세 수납"/>
