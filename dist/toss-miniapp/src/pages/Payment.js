...
const PaymentBottomSheet_1 = require("../components/PaymentBottomSheet");
function Payment() {
    const [bank, setBank] = (0, react_1.useState)('국민');
    const [account, setAccount] = (0, react_1.useState)('11111111111');
    const [amount, setAmount] = (0, react_1.useState)('500000');
    const [isSheetOpen, setSheetOpen] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
    ...
    const handleTossSend = () => {
        ...
        try {
            ...
            const isTossApp = /Toss/i.test(navigator.userAgent) || !!window.Toss;
            if (isTossApp) {
                // 미니앱 공식 SDK 송금 브릿지 호출 (app-in-toss/web-bridge 대용)
                Promise.resolve().then(() => __importStar(require('@apps-in-toss/web-bridge'))).then((m) => {
                    const bridge = m;
                    ...
                    if (bridge && typeof bridge.sendMoney === 'function') {
                        ...
                        }).catch((err) => console.error('송금 브릿지 에러:', err));
                    }
    ...
    return (<div style={{ padding: '24px' }}>
      ...
      {/* 결제 수단 팝업을 여는 메인 버튼 */}
      <tds_1.Button variant="primary" onClick={() => setSheetOpen(true)}>
      ...
      <PaymentBottomSheet_1.PaymentBottomSheet isOpen={isSheetOpen} onClose={() => setSheetOpen(false)} onSelectTossTransfer={handleTossSend} onSelectTossPay={handleTossPayments} onSelectManualDeposit={handleManualDeposit}/>
    </div>);
}
