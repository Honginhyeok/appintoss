...
function getPaymentStatus(tenant, transactions) {
    ...
    const rentTxs = transactions.filter(t => {
        var _a;
        return t.type === 'INCOME' && t.category === 'RENT' &&
            ((_a = t.description) === null || _a === void 0 ? void 0 : _a.includes(tenant.name)) &&
...
function PaymentStatus() {
    ...
    const load = (0, react_1.useCallback)(() => __awaiter(this, void 0, void 0, function* () {
        setLoading(true);
        try {
            const [tRes, txRes] = yield Promise.all([
                (0, env_1.apiFetch)('/api/tenants'),
                (0, env_1.apiFetch)('/api/transactions'),
            ]);
            if (tRes.ok && txRes.ok) {
                const tenants = yield tRes.json();
                const txs = yield txRes.json();
    ...
    return (<div style={{ padding: '24px', minHeight: '100vh', background: '#fff' }}>
      ...
      {/* 요약 뱃지 */}
      {isLandlord && (<div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
          {[
            ...
            ].map(b => (<div key={b.label} style={{ flex: 1, background: b.bg, borderRadius: '14px', padding: '14px', textAlign: 'center' }}>
      ...
      {/* 리스트 */}
      {loading ? (<div style={{ textAlign: 'center', padding: '40px', color: '#8b95a1' }}>불러오는 중...</div>) : data.length === 0 ? (<div style={{ textAlign: 'center', padding: '60px 20px', color: '#8b95a1' }}>
          ...
          {data.map(({ tenant, status, lastPaid }) => {
                ...
                return (<div key={tenant.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#8b95a1' }}>
                    {((_a = tenant.room) === null || _a === void 0 ? void 0 : _a.name) || '미배정'}
